...
public class OrderController {
    ...
    // Cambiar estado del pedido con manejo de reglas de negocio
    @PatchMapping("/{id}/status")
    public ResponseEntity<?> cambiarEstado(@PathVariable Long id, @RequestParam OrderStatus status) {
        try {
            Order pedidoActualizado = orderService.cambiarEstado(id, status);
            return ResponseEntity.ok(pedidoActualizado);
        } catch (IllegalStateException e) {
            // Captura las reglas de transición no permitidas
